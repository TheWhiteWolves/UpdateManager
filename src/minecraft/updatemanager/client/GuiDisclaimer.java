...
public class GuiDisclaimer extends GuiScreen {
	...
	public void initGui() {
		...
		if (disclaimer.length < 1 || MathHelper.stringNullOrLengthZero(header = disclaimer[0])) actionPerformed(noDown);
		super.initGui();
	}

	@Override
	protected void actionPerformed(GuiButton button) {
		if (button.id == 0) if (!ThreadDownloadMod.downloadings.contains(mod.getModName())) new ThreadDownloadMod(mod.getDirectDownloadURL(), mod);
	...
	public void drawScreen(int par1, int par2, float par3) {
		...
		drawCenteredString(fontRenderer, header, width / 4, 5, 0xFF0000);
